  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        ...
        >
          {exhibition.tokensLeft}/{exhibition.totalTokens} Available
        </span>
      </div>
      ...
      >
        ...
        >
          {(exhibition.price / 100000).toFixed(0)} Lakhs
        </span>
      </div>
...
export default function FleetIndex({ onSelect }: FleetIndexProps) {
  const sectionRef = useRef<HTMLElement>(null);
  const headerRef = useRef<HTMLDivElement>(null);

  if (!fleetConfig.sectionLabel && exhibitions.length === 0) {
    return null;
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {fleetConfig.sectionLabel}
        </span>
        {fleetConfig.countLabel && (
          <span
            style={{
              fontFamily: 'var(--font-sans)',
              fontSize: '0.7rem',
      ...
      >
        {exhibitions.map((ex) => (
          <FleetItem key={ex.slug} exhibition={ex} onSelect={onSelect} />
        ))}
      </div>
    </section>
  );
